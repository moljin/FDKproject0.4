...
@admin_products_bp.route('/shop/category/list', methods=['GET', 'POST'])
@admin_required
def shop_list():
...
@admin_products_bp.route('/shop/category/<int:_id>/change', methods=['GET', 'POST'])
@admin_required
def shop_change(_id):
    form = ShopCategoryForm()
...
@admin_products_bp.route('/shop/category/create', methods=['GET', 'POST'])
@admin_required
def shop_create():
    form = ShopCategoryForm()
    user_objs = User.query.all()
...
@admin_products_bp.route('/shop/category/save', methods=['POST'])  # 프로필이미지경로: profile_images, 등록증경로: profile
@admin_required
def save():
    if request.method == 'POST':
        ...
        if target_shop:
            ...
            db.session.commit()
            return redirect(url_for("admin_products.shop_change", _id=new_shop.id))
    else:
        abort(404)
